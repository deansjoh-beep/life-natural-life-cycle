                ...
                <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm md:col-span-2 no-print">
                  ...
                  <p className="mt-3 text-[11px] text-gray-400 flex items-center gap-1">
                    <Info size={12} /> {(saju && saju.adjusted ? '월지 조정: 일간 [' + saju.dayMaster + ']와 월지 [' + saju.monthBranch + ']의 음양이 달라 60갑자에 없는 조합이므로, 월지를 [' + saju.baseBranch + ']로 조정해 기점 후보를 잡았습니다. ' : '') + (decision && decision.ipchunBranch && saju
                      ? (decision.method === '조후'
                          ? '조후 판정: 사주가 ' + decision.johu.chart + (decision.johu.chart === '한' ? '(寒)' : '(暖)') + '하여 ' + (decision.johu.chart === '한' ? '따뜻한 쪽' : '차가운 쪽') + '인 [' + decision.ipchuBranch + ']가 조후용신에 가까움 → [' + saju.dayMaster + decision.ipchuBranch + ']가 입추(정점)이 되고, [' + saju.dayMaster + decision.ipchunBranch + ']를 입춘 기점으로 추천합니다. 살아온 인생과 다르면 직접 변경하세요.' + (saju.timeBranch === null ? ' 지금은 월지만으로 판정한 결과라, 출생 시간을 입력하면 판정이 중립으로 바뀔 수 있습니다. 추천이 살아온 인생과 어긋나면 가이드의 "기점을 직접 골라야 할 때"를 참고해 직접 확인해 보세요.' : '')
